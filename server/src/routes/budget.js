...
export function budgetRouter(db) {
  ...
  r.get('/summary', (req, res) => {
    const lines = db.prepare('SELECT * FROM budget_line ORDER BY sort_order, id').all();
    const hospital = db.prepare('SELECT awarded_amount, requested_amount FROM hospital WHERE id = 1').get();
