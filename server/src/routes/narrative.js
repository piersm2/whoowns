...
function context(db) {
  ...
  const answers = Object.fromEntries(db.prepare('SELECT key, answer FROM intake_answer').all().map((a) => [a.key, a.answer]));
  const budget = db.prepare('SELECT * FROM budget_line ORDER BY sort_order, id').all();
...
export function narrativeRouter(db) {
  ...
  r.put('/answers/:key', (req, res) => {
    ...
    db.prepare("INSERT INTO intake_answer (key, answer) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET answer = excluded.answer, updated_at = datetime('now')").run(req.params.key, parsed.data.answer);
    res.json({ key: req.params.key, answer: parsed.data.answer });
  });

  r.get('/sections', (req, res) => res.json(db.prepare('SELECT * FROM narrative_section ORDER BY sort_order').all()));
  ...
  r.patch('/sections/:key', (req, res) => {
    ...
    if (data.content !== undefined && data.status === undefined) {
      const current = db.prepare('SELECT status FROM narrative_section WHERE key = ?').get(req.params.key);
    ...
    const info = db.prepare(`UPDATE narrative_section SET ${keys.map((k) => `${k} = ?`).join(', ')}, updated_at = datetime('now') WHERE key = ?`).run(...keys.map((k) => data[k]), req.params.key);
    if (!info.changes) return res.status(404).json({ error: 'Not found' });
    logActivity(db, 'narrative_section', req.params.key, 'update', keys.join(','));
    res.json(db.prepare('SELECT * FROM narrative_section WHERE key = ?').get(req.params.key));
  });

  // Generate a draft. Does not overwrite unless ?apply=1 is passed.
  r.post('/sections/:key/draft', (req, res) => {
    const section = db.prepare('SELECT * FROM narrative_section WHERE key = ?').get(req.params.key);
    if (!section) return res.status(404).json({ error: 'Not found' });
    const draft = draftSection(section.key, context(db));
    if (req.query.apply === '1') {
      db.prepare("UPDATE narrative_section SET content = ?, status = 'draft', updated_at = datetime('now') WHERE key = ?").run(draft, section.key);
      logActivity(db, 'narrative_section', section.key, 'draft', 'generated');
    }
    res.json({ key: section.key, draft, placeholders: (draft.match(/\[[^\]]+\]/g) || []).length });
  });

  r.get('/export.md', (req, res) => {
    const { hospital, categories } = context(db);
    const sections = db.prepare('SELECT * FROM narrative_section ORDER BY sort_order').all();
