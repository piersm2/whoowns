...
export function crudRouter(db, table, schema, { orderBy = 'id', onChange } = {}) {
  ...
  r.post('/', (req, res) => {
    ...
    logActivity(db, table, info.lastInsertRowid, 'create', row.title ?? row.line_item ?? row.description ?? '');
    onChange?.(row);
  ...
  r.patch('/:id', (req, res) => {
    ...
    const info = db.prepare(`UPDATE ${table} SET ${sets.join(', ')} WHERE id = ?`).run(...keys.map((k) => data[k]), req.params.id);
    if (!info.changes) return res.status(404).json({ error: 'Not found' });
    const row = db.prepare(`SELECT * FROM ${table} WHERE id = ?`).get(req.params.id);
    logActivity(db, table, req.params.id, 'update', keys.join(','));
    onChange?.(row);
    res.json(row);
  });
  ...
  r.delete('/:id', (req, res) => {
    ...
    if (!info.changes) return res.status(404).json({ error: 'Not found' });
    logActivity(db, table, req.params.id, 'delete');
    res.status(204).end();
  });
